...
#define UART_PRINT_PERIOD_MS  500U
/* USER CODE END PD */
...
static volatile uint16_t ldr_average = 0U;
/* USER CODE END PV */
...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
static uint16_t adc_calculate_average(
...
static void uart_print_adc_value(uint16_t value)
{
    char message[64];
...
int main(void)
{
  ...
  while (1)
  {
        if (adc_first_half_ready != 0U)
        {
            ...
            ldr_average = adc_calculate_average(
                &adc_buffer[0],
        ...
        if (adc_second_half_ready != 0U)
        {
            ...
            ldr_average = adc_calculate_average(
                &adc_buffer[ADC_HALF_BUFFER_SIZE],
                ADC_HALF_BUFFER_SIZE
            );
        }
        ...
        if ((current_tick - last_uart_tick) >= UART_PRINT_PERIOD_MS)
        {
            last_uart_tick = current_tick;
            uart_print_adc_value(ldr_average);
        }
    /* USER CODE END WHILE */
